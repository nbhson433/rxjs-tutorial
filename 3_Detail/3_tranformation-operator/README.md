...
Nếu bạn dùng với RxJS version < 5.5 thì có thể các bạn sẽ thấy cú pháp sử dụng khác là prototype method chain/dot-chain (observable.map().takeUntil()), nhưng nếu bạn dùng từ version 5.5 trở lên thì nên dùng pipe operators, dựa theo một số giải thích ở đây: [pipeable operators](https://rxjs.dev/guide/v6/pipeable-operators)
...
```
`Nếu không tồn tại thì giá trị nhận được là undefined`
...
Sẽ thế nào nếu bạn muốn bất cứ khi nào stream emit một giá trị thì bạn luôn trả về một giá trị fixed (mặc định) không?
Ví dụ khi có một event xảy ra thì mình muốn có giá trị này,  không quan tâm event đó trả về giá trị gì.
